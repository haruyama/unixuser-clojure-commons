...
(deftest test-make-query
         (let
           [
            query (mixi.solr.solrj/make-query nil 0 0 [["timestamp" "asc"] ["score" "desc"]])
            ]
           (is (= "*:*" (. query getQuery)))
           (is (= 0 (. query getStart)))
           (is (= 0 (. query getRows)))
         ...
         (let
           [
            query (mixi.solr.solrj/make-query "hoge:lupin" 20 10 nil)
            ]
         ...
         (let
           [
            query (mixi.solr.solrj/make-query nil 30 10 nil [["facet" "true"] ["facet.field" "category_id"]])
            ]
           (is (= "*:*" (. query getQuery)))
         ...
         (let
           [
            query (mixi.solr.solrj/make-query nil 30 10 nil [["facet" "true"] ["facet.field" ["category_id" "date"]]] [["hl" "true"] ["hl.fl" ["body" "title"]]])
            ]
           (is (= "*:*" (. query getQuery)))
           (is (= 30 (. query getStart)))
           (is (= 10 (. query getRows)))
